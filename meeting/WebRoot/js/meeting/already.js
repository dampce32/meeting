$(document).ready(function(){
	...
	var selectRow = null;
	$(dataList).datagrid({  
		url:'Meeting.do?action=queryAlready',
	    rownumbers:true,
	    fitColumns:true,
	    singleSelect:true,
	    fit:true,
	...
	var onJoin = function(){
		if(selectRow == null){
			$.messager.alert('提示','请选中您要取消参加的会议','info');
			return;
		}
		$.messager.confirm('确认', '您确定取消参加这个会议吗？', function(r){
			if(r){
				var data = 'meetingId='+selectRow.meetingId+'&action=cancel';
				$.ajax({
					   type: "POST",
					   url: "Meeting.do",
					   data:data,
